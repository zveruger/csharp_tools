...
namespace Tools.CSharp.Controls
{
    ...
    {
        #region private
        private readonly static ProfessionalColorTable _ColorTable = new ProfessionalColorTable();
        //---------------------------------------------------------------------
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (mainRect.Height != 0 && mainRect.Width != 0)
            {
                ...
                if (BorderStyle == BorderStyle.None && _renderBorder)
                {
                    using (var p = new Pen(_ColorTable.ToolStripBorder))
                    { graphics.DrawLine(p, mainRect.Left, mainRect.Height - 1, mainRect.Right, mainRect.Height - 1); }
                }
            }
    ...
    //-------------------------------------------------------------------------
    internal sealed class GradientPanelDesigner : ControlDesigner
    {
        #region private
        ...
        {
            get { return LazyInitializer.EnsureInitialized(ref _actionList, () => new DesignerActionListCollection { new DesignerGradientPanelActionList(Control)}); }
        }
        public override DesignerVerbCollection Verbs
        {
            get { return LazyInitializer.EnsureInitialized(ref _verbs, () => new DesignerGradientPanelVerbCollection(Control)); }
        }
        //---------------------------------------------------------------------
    }
    //-------------------------------------------------------------------------
    internal sealed class DesignerGradientPanelActionList : DesignerActionList
    {
        #region private
        private readonly GradientPanel _control;
    ...
    //-------------------------------------------------------------------------
    internal sealed class DesignerGradientPanelVerbCollection : DesignerVerbCollection
    {
        #region private
        private readonly GradientPanel _control;
        //---------------------------------------
