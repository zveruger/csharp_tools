...
namespace Tools.CSharp.Extensions
{
    public static class AssemblyExtensions
    {
        ...
        public static Version AssemblyFileVersion(this Assembly assembly)
        {
            if (assembly == null)
            { throw new ArgumentNullException(nameof(assembly)); }

            var attributes = assembly.GetCustomAttributes(typeof(AssemblyFileVersionAttribute), false);
            return (attributes.Length != 0) ? new Version(((AssemblyFileVersionAttribute)attributes[0]).Version) : new Version();
        }
